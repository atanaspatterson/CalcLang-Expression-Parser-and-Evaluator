...
bool scan_is_hexlitchar(char c) {
...
//scans hexilt and binlit tokens
char * scan_hexlit_binlit(char *p, char *end, struct scan_token_st *tp) {
	if (*(p + 1) == 'x')
		tp->id = TK_HEXLIT;
	else if (*(p + 1) == 'b')
		tp->id = TK_BINLIT;
    ...
    int count = 0;
    while (scan_is_digit(*p) || scan_is_hexlitchar(*p)) {
        tp->name[count] = *p;
        p += 1;
        count += 1;
    }
...
char * scan_token(char *p, char *end, struct scan_token_st *tp) {
    ...
    } else if (*p == '0' && !scan_is_digit(*(p + 1))){
    	p = scan_hexlit_binlit(p, end, tp);
